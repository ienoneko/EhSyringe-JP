...
merge(
    ...
    undefined,
    {
        ...
        'While tagging is relatively straight-forward, there are a number of established guidelines that determine when adding a particular tag is proper and when it is not. Before you put any significant amount of effort into it, we therefore ask that you skim through the ':
            '虽然打标签相对简单，但有许多既定准则可以确定何时添加特定标签是正确的。因此，在您投入其中之前，我们要求您阅读',
        ...
        '. This will likely save both you and the tagging moderators from doing unnecessary work. In particular, you should note the following:':
            '。这能有效避免您和标签版主进行不必要的工作。特别需要注意以下几点：',
        ...
        'Uploader Comment': 'UP主より',
        'Score ': '点数 ',
        'Vote+': '高評価する',
        'Vote-': '低評価する',
        'Last edited on ': '最后编辑于 ',
        'click to show all': '显示全部',
        'Post New Comment': 'コメントを書く',
    ...
    [
        ...
        [/^French\s*$/, 'フランス語 \xA0'],
        [/^German\s*$/, 'ドイツ語 \xA0'],
        [/^Hungarian\s*$/, '匈牙利语 \xA0'],
        [/^Italian\s*$/, '意大利语 \xA0'],
        [/^Korean\s*$/, '韓国語 \xA0'],
        [/^Polish\s*$/, '波兰语 \xA0'],
        [/^Portuguese\s*$/, '葡萄牙语 \xA0'],
        [/^Russian\s*$/, 'ロシア語 \xA0'],
        [/^Spanish\s*$/, '西班牙语 \xA0'],
        [/^Thai\s*$/, 'タイ語 \xA0'],
        [/^Vietnamese\s*$/, 'ベトナム語 \xA0'],
...
merge(
    ...
    undefined,
    {
        ...
        'You must specify a valid expunge type to start new expunge petition.':
            '您必须选择一个删除类型才能提交新的删除申请。',
        'You must provide a reason to start a new expunge petition.': '您必须提供一个理由才能提交新的删除申请。',
        'You must provide a link to the conflicting gallery to start a new expunge petition of this type.':
            '您必须提供冲突图库的链接才能提交此类型的删除申请。',
...
merge(
    ...
    undefined,
    {
        ...
        'Provide any relevant details. If the gallery has excessive updates that are not allowed under the community guidelines, it will be locked for further updates.':
            '提供任何相关细节。图库频繁更新不符合社区指南，会被锁定以防止继续更新。',
        ...
        'Provide any relevant details. If valid content has been removed from the gallery, it will be reverted. If valid content has been replaced with other valid content, it will be split into two separate galleries.':
            '提供任何相关细节。如果图库中的有效内容被删除，则会被恢复。如果有效内容被其他有效内容替换，将被拆分成两个单独的图库。',
    },
    [],
);
